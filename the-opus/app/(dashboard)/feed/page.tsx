  ...
  return (
    <div className="flex flex-col min-h-screen">
      <div className="flex flex-col xl:flex-row flex-1 overflow-hidden">
          ...
          <div>
            <div className="mb-10">
              <h3 className="flex items-center gap-2 text-xs font-bold mb-4 uppercase tracking-[0.2em] text-slate-500">
                <Sparkles size={14} className="text-[#7c5cff]" /> Trending Now
              </h3>
            ...
            <h2 className="text-2xl font-bold mb-1 text-[#15161a]" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Welcome, {firstName}!</h2>
            <p className="text-slate-500 mb-6 text-sm uppercase tracking-widest">Explore all Agents</p>

            {/* Scope filter */}
            <div className="mb-8 inline-flex rounded-lg border border-slate-200 bg-white p-1 text-xs font-bold">
              <Link
                href="/feed"
                className={`flex items-center gap-1.5 rounded-md px-3 py-1.5 transition-colors ${!openOnly ? 'bg-[#7c5cff] text-white' : 'text-slate-500 hover:text-[#15161a]'}`}
              >
                All agents
              </Link>
              <Link
        ...
        <aside className="w-full xl:w-80 border-t xl:border-t-0 xl:border-l border-slate-200 p-6 bg-white/60">
...
const TrendingCard = ({ agent, rank }: any) => (
  <Link href={`/agent/${agent.id}`} className="relative group p-4 rounded-xl bg-white border border-slate-200 shadow-sm hover:border-[#7c5cff]/50 transition-all cursor-pointer overflow-hidden block">
    <div className="absolute top-0 right-0 p-2 opacity-[0.06] group-hover:opacity-10 transition-opacity">
      <TrendingUp size={40} />
    </div>
    <div className="relative z-10 flex items-center gap-3">
      ...
      <div className="min-w-0">
        <h4 className="text-sm font-bold text-[#15161a] truncate">{agent.name}</h4>
...
const AgentCard = ({ id, name, author, avatarUrl, tags, description, likes, downloads, model, featured, restricted, department, locked }: any) => (
  <Link href={`/agent/${id}`} className={`p-6 rounded-xl border transition-all shadow-sm hover:border-[#7c5cff]/40 group flex flex-col ${
    featured
      ? 'bg-[#7c5cff]/5 border-[#7c5cff]/40 ring-1 ring-[#7c5cff]/30'
      : 'bg-white border-slate-200'
  }`}>
    <div className="flex justify-between items-start mb-4 gap-2">
      <div className="flex items-center gap-3">
        <div className="relative w-10 h-10 shrink-0 rounded-full border border-slate-200 overflow-hidden bg-slate-100">
          {avatarUrl
        ...
        <div className="min-w-0">
          <h4 className="font-bold leading-tight group-hover:text-[#7c5cff] transition-colors truncate text-[#15161a]">{name}</h4>
          <p className="text-xs text-slate-500 truncate">by {author}</p>
        </div>
      </div>
      <div className="flex items-center gap-1.5 shrink-0">
        {restricted && department && (
          <span className={`flex items-center gap-1 px-2 py-1 rounded text-[10px] font-bold whitespace-nowrap border ${locked ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'}`}>
    ...
    <div className="flex flex-wrap gap-2 mb-6">
      {tags.map((t: string) => (
        <span key={t} className="px-2 py-0.5 bg-slate-100 text-[10px] font-bold rounded text-slate-600 border border-slate-200">
          {t}
        </span>
      ))}
    </div>
    ...
    <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-200">
      ...
      <span className="bg-[#15161a] group-hover:bg-black text-white text-xs font-bold px-4 py-2 rounded transition-all">
        VIEW
      </span>
    </div>
  </Link>
